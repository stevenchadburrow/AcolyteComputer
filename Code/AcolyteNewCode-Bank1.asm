...
; First, run
; ~/dev65/bin/as65 AcolyteNewCode-Bank1.asm

; Second, run
; ./Parser.o AcolyteNewCode-Bank1.lst AcolyteNewCode-Bank1.bin 32768 32768 32768 0

; Third (for second bank), run
; ~/dev65/bin/as65 AcolyteNewCode-Bank2.asm

; Fourth (for second bank), run
; ./Parser.o AcolyteNewCode-Bank2.lst AcolyteNewCode-Bank2.bin 32768 32768 32768 0

; Fifth, run
; ./Combiner.o VideoROM.bin AcolyteNewCode-Bank1.bin AcolyteNewCode-Bank2.bin AcolyteCode-128K.bin AcolyteCode-512K.bin

; OR, all together
; ~/dev65/bin/as65 AcolyteNewCode-Bank1.asm ; ./Parser.o AcolyteNewCode-Bank1.lst AcolyteNewCode-Bank1.bin 32768 32768 32768 0 ; ~/dev65/bin/as65 AcolyteNewCode-Bank2.asm ; ./Parser.o AcolyteNewCode-Bank2.lst AcolyteNewCode-Bank2.bin 32768 32768 32768 0 ; ./Combiner.o VideoROM.bin AcolyteNewCode-Bank1.bin AcolyteNewCode-Bank2.bin AcolyteCode-128K.bin AcolyteCode-512K.bin
...
; Keyboard can be turned off by doing
; STZ key_status
; CLI
; Those together will lock the keyboard functionality,
; but allow /IRQ to be used by some other source.
; To turn the keyboard back on you would
; LDA #$01
; STA key_status
; SEI
...
jump_vector_irq_extra	.EQU $03A0 ; 16 bytes here
...
vector_reset
	SEI			; turn off interrupts from /IRQ, thus turning ON the keyboard
...
wipeout_sys_loop
	...
	LDX #$00
	JMP main_display_loop
main_display_text
	.BYTE "F12 for"
	.BYTE " Help",$0D
main_display_loop
	LDA main_display_text,X
	JSR printchar
	INX
	CPX #$0D
	BNE main_display_loop
...
monitor
	...
	PHY
	LDX #$00
monitor_command_copy
	LDA command_array,X
	STA command_array_prev,X
	INX
	CPX #$28 
	BNE monitor_command_copy
	STZ command_mode
	LDY #$00
	STZ command_place
...
; in the past, it would literally wait until it had a non-$FF value, forever!
...
basic_line_number_prompt
	; start of inserted text for expanded RAM and I/O
	PHA
	STZ $1FFF ; off screen (overscan)
	STZ $9FFF ; bottom of expanded RAM
	LDA #$FF ; load non-zero value
	STA $9FFF ; store in expanded RAM
	LDA $1FFF ; check screen RAM
...
basic_memory_expansion
	PLA
	; end of inserted text for expanded RAM and I/O
	CLC
...
ascii_lookup
	.BYTE $00,$16,$1F,$0C,$1E,$1C,$1D,$15
	.BYTE $00,$18,$07,$0E,$0F,$09,$60,$00
...
printchar_continue10
	CMP #$0E ; shift out (F6, always shifts)
...
printchar_continue15
	CMP #$0F ;  shift in (F4, soft reset)
	BNE printchar_continue16
	LDA key_alt_control
	BEQ printchar_continue16
	JMP main_start ; doesn't wipe BASIC memory
printchar_continue16
	CMP #$15 ; neg ack (F12, help menu)
...
printchar_continue19
	...
	LDA #$03 ; basic mode
	STA command_mode
	JMP printchar_command_zero
printchar_continue20
	CMP #$16 ; sync idle (F9, tetra game)
	BNE printchar_continue21
	LDA key_alt_control
	BEQ printchar_continue21
	; put jump code here

	LDY #$C0 ; location of tetra on Bank 2
	LDX #$00
	JMP switch_banks ; switch banks

	JMP printchar_exit
printchar_continue21
	CMP #$18 ; cancel (F10, intruders game)
	BNE printchar_continue22
	LDA key_alt_control
	BEQ printchar_continue22
	; put jump code here

	LDY #$D0 ; location of intruders on Bank 2
	LDX #$00
	JMP switch_banks ; switch banks

	JMP printchar_exit
printchar_continue22
	CMP #$19 ; end of medium (F11, load SDcard)
...
printchar_continue23
	CMP #$1A ; insert
	BNE printchar_continue24
	LDA command_mode
	CMP #$01 ; only in monitor mode
	BNE printchar_continue24
	LDX #$02
printchar_command_paste
	LDA command_array_prev,X
	JSR printchar
	INX
	CPX #$28
	BNE printchar_command_paste
	JMP printchar_exit
printchar_continue24
	CLC
